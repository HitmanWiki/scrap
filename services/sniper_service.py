...
class SniperService:
    ...
    async def execute_sell(self, wallet: Keypair, token_mint: str, amount_tokens: float, slippage_bps: int) -> dict:
        """Sell via Jupiter - same way Phantom does it"""
        try:
            decimals = await self.get_token_decimals(token_mint)
            amount_raw = int(amount_tokens * 10**decimals)
            
            print(f"   💱 Jupiter sell: {amount_tokens:.2f} tokens...")
            
            # Jupiter v6 API (same as Phantom uses)
            ...
            for api_base in api_urls:
                try:
                    ...
                    # Handle both v1 and v6 response formats
                    out_amount = quote.get("outputAmount") or quote.get("outAmount", "0")
                    if out_amount == "0" or not out_amount:
                        continue
                    
                    expected_sol = int(out_amount) / 1e9 if "outputAmount" in quote else int(out_amount) / 1e9
                    print(f"   ✅ Route found: {expected_sol:.6f} SOL")
                    
                    # Build swap
                    swap_url = f"{api_base}/swap"
                    payload = {
                        "quoteResponse": quote,
            ...
            return {"success": False, "error": "No routes found on any API"}
