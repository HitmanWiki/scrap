...
class SniperService:
    ...
    async def execute_sell(self, wallet: Keypair, token_mint: str, amount_tokens: float, slippage_bps: int) -> dict:
        """Sell using DexScreener pool discovery + PumpSwap direct"""
        try:
            ...
            # Step 1: Get pool from DexScreener
            pool_address = None
            dex_name = None
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if data.get('pairs') and len(data['pairs']) > 0:
                        ...
                        dex_name = best_pair.get('dexId', '').lower()
                        print(f"   📊 Pool: {pool_address} on {dex_name}")
            except:
                pass
            
            if not pool_address:
                return {"success": False, "error": "No pool found on DexScreener"}
            
            # Step 2: If it's PumpSwap, use PumpSwap program
            if 'pump' in dex_name:
                ...
                # Derive vaults from the pool
                token_vault, _ = Pubkey.find_program_address(
                    [b"vault", bytes(pool), bytes(mint)], PUMP_SWAP
                )
                sol_vault, _ = Pubkey.find_program_address(
                    [b"vault", bytes(pool), bytes(SOL_MINT)], PUMP_SWAP
                )
                
                # Try swap instruction
                SWAP_DISC = bytes([0xe5, 0x17, 0xcb, 0x97, 0x7a, 0xe3, 0xad, 0x2a])
                
                data = SWAP_DISC
                data += amount_raw.to_bytes(8, 'little')
                data += int(0).to_bytes(8, 'little')
            ...
            # Step 3: For other DEXes, try Jupiter Lite
            for slip in [slippage_bps, 5000, 10000]:
